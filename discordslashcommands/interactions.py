...
class Interaction:
    ...
    async def end(self, *args, **kwargs):
        requests.post(f"https://discord.com/api/v8/interactions/{self.id}/{self.token}/callback", 
        headers = { "Authorization": "Bot {self._client.http.token}" },
        json = { "type": 4, "data": kwargs }
    )
